        ...
        return (
          <button
            ...
            className={`w-full flex items-center gap-3 p-3 rounded-2xl border-2 transition-all ${
              active ? 'border-blue-600 bg-blue-50' : 'border-gray-100 bg-white hover:border-gray-300'
            }`}
          >
            ...
            <div className="flex-1 text-left">
              ...
              <div className="text-xs text-gray-500">{opt.description} · bis {opt.maxPassengers} Pers.</div>
            </div>
            <div className="text-right shrink-0">
              <div className={`font-bold text-base ${active ? 'text-blue-700' : 'text-gray-900'}`}>
                {price.toFixed(2)} €
              </div>
              <div className="text-xs text-gray-400">ca. {opt.eta} Min.</div>
            </div>
          </button>
        )
      })}
