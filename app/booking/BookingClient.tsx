  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-3 flex items-center gap-3 sticky top-0 z-20">
        {step > 0 && step < 4
          ? <button onClick={() => setStep(s => s - 1)} className="p-1"><ArrowLeft className="w-5 h-5 text-gray-700" /></button>
          : <Link href="/" className="p-1"><ArrowLeft className="w-5 h-5 text-gray-700" /></Link>}
        <div className="flex-1">
          <div className="font-semibold text-gray-900 text-sm">{STEPS[step]}</div>
      ...
      <div className="max-w-lg mx-auto px-4 py-5 space-y-4">
        ...
        {step === 0 && (
          <>
            ...
            <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide block mb-1">Abholzeit</label>
              <input type="datetime-local" min={today} value={scheduledAt} onChange={e => setScheduledAt(e.target.value)}
                className="w-full h-12 px-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-blue-500" />
            </div>
          </>
        )}
        ...
        {step === 1 && (
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 space-y-3">
            <h2 className="font-semibold text-gray-900">Ihre Daten</h2>
            {[
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide flex items-center gap-1 mb-1"><Users className="w-3 h-3" />Personen</label>
                <select value={passengers} onChange={e => setPassengers(Number(e.target.value))}
                  className="w-full h-12 px-3 rounded-xl border border-gray-200 text-sm bg-white focus:outline-none focus:border-blue-500">
                  {[1,2,3,4,5,6,7,8].map(n => <option key={n} value={n}>{n}</option>)}
                </select>
              </div>
              <div>
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide block mb-1">🧳 Gepäck</label>
            ...
            <div className="border-t border-gray-100 pt-3">
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide flex items-center gap-1 mb-2"><Plane className="w-3 h-3" />Flugdaten (optional)</label>
              <div className="grid grid-cols-2 gap-2">
                <input type="text" value={flightNumber} onChange={e => setFlightNumber(e.target.value)} placeholder="LH 1234"
                  className="h-11 px-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-blue-500" />
                <input type="datetime-local" value={flightTime} onChange={e => setFlightTime(e.target.value)}
                  className="h-11 px-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-blue-500" />
              </div>
            </div>
            <div>
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide block mb-1">Hinweise (optional)</label>
        ...
        {step === 2 && (
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
            <h2 className="font-semibold text-gray-900 mb-1">Fahrzeugklasse wählen</h2>
            <p className="text-xs text-gray-400 mb-3">Strecke: ca. {distanceKm.toFixed(1)} km</p>
        ...
        {step === 3 && (
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 space-y-3">
            <h2 className="font-semibold text-gray-900">Zahlungsart</h2>
            {([
              ...
              <button key={method} onClick={() => setPaymentMethod(method)}
                className={`w-full flex items-center gap-3 p-4 rounded-2xl border-2 transition-all ${paymentMethod === method ? 'border-blue-600 bg-blue-50' : 'border-gray-100 hover:border-gray-300'}`}>
                <div className={`p-2 rounded-xl ${paymentMethod === method ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600'}`}>{icon}</div>
                <div className="text-left flex-1">
                  <div className="font-semibold text-gray-900 text-sm">{label}</div>
                  <div className="text-xs text-gray-400">{sub}</div>
                </div>
                {paymentMethod === method && <div className="w-5 h-5 bg-blue-600 rounded-full flex items-center justify-center text-white text-xs">✓</div>}
              </button>
            ))}
            <div className="border-t border-gray-100 pt-3 space-y-1.5">
              <div className="flex justify-between text-sm"><span className="text-gray-500">Abholung</span><span className="text-gray-900 text-right max-w-52 truncate">{pickup?.address}</span></div>
              <div className="flex justify-between text-sm"><span className="text-gray-500">Ziel</span><span className="text-gray-900 text-right max-w-52 truncate">{dropoff?.address}</span></div>
              <div className="flex justify-between text-sm"><span className="text-gray-500">Abholzeit</span><span className="text-gray-900">{scheduledAt ? new Date(scheduledAt).toLocaleString('de-DE', { dateStyle: 'short', timeStyle: 'short' }) : '–'}</span></div>
              <div className="flex justify-between text-sm"><span className="text-gray-500">Klasse</span><span className="capitalize">{rideClass}</span></div>
              <div className="flex justify-between font-bold text-base border-t border-gray-100 pt-2 mt-1">
                <span>Gesamt</span><span className="text-blue-700">{price.toFixed(2)} €</span>
              </div>
            </div>
          </div>
        )}
        ...
        {step === 4 && (
          <div className="text-center py-8">
            ...
            <h2 className="text-xl font-bold text-gray-900 mb-2">Buchung bestätigt!</h2>
            <p className="text-gray-500 text-sm mb-2">Buchungs-ID:</p>
            <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 inline-block font-mono font-bold text-blue-800 text-lg mb-4">{bookingId}</div>
            <p className="text-gray-500 text-sm mb-6">Bestätigung wurde an <strong>{passengerEmail}</strong> gesendet.</p>
            <div className="space-y-2">
              <Button onClick={() => router.push(`/track/${bookingId}`)} className="w-full bg-blue-600 hover:bg-blue-700 text-white rounded-2xl h-12">Fahrt verfolgen →</Button>
              <Button onClick={() => router.push('/')} variant="outline" className="w-full rounded-2xl h-12">Zurück zur Startseite</Button>
            </div>
          </div>
        )}
        ...
        {step < 4 && (
          <Button onClick={step === 3 ? handleBook : () => setStep(s => s + 1)} disabled={!canNext() || loading}
            className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-2xl h-14 text-base disabled:opacity-40">
